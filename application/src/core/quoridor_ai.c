...
{
    clock_t clk = clock();
    return clk / (CLOCKS_PER_SEC / 1000);
}
#endif
...
static bool QuoridorCore_canPlayTurn(QuoridorCore *self, QuoridorTurn turn)
{
    switch (turn.action)
    {
    case QUORIDOR_PLAY_HORIZONTAL_WALL:
        return QuoridorCore_canPlayWall(self, WALL_TYPE_HORIZONTAL, turn.i, turn.j);
...
static float QuoridorCore_minMax(QuoridorCore *self, int playerID, int currDepth, int maxDepth, float alpha, float beta, QuoridorTurn *turn, void *AIdata)
{
    ...
    if (currDepth >= maxDepth)
        return QuoridorCore_computeScore(self, playerID, *turn);
    ...
    if (QuoridorCore_canPlayTurn(self, *turn))
    {
...
#if DEBUG == 0
    // for (int maxDepth = 0; maxDepth <= depth; maxDepth++)
    // {
    //     childValue = QuoridorCore_minMax(self, self->playerID, 0, maxDepth, alpha, beta, &childTurn, aiData);
    //     if (childTurn.action != QUORIDOR_ACTION_UNDEFINED && childValue > bestValue)
    //     {
