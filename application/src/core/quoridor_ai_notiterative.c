...
void QuoridorCore_other_getShortestPath(QuoridorCore *self, int playerID, QuoridorPos *path, int *size);
...
float QuoridorCore_other_computeScore(QuoridorCore *self, int playerID, QuoridorTurn turn)
{
    ...
    QuoridorCore_other_getShortestPath(self, playerA, my_path.tiles, &(my_path.size));
    QuoridorCore_other_getShortestPath(self, playerB, other_path.tiles, &(other_path.size));
    ...
    // Chemin de l'adversaire simple et court
...
static float QuoridorCore_other_computeWall(QuoridorCore self, int playerID, QuoridorPath my_path, QuoridorPath other_path, QuoridorTurn turn)
{
    ...
    if (turn.action == QUORIDOR_PLAY_VERTICAL_WALL)
    {
        ...
        {
            if (self.vWalls[i][turn.j] != WALL_STATE_NONE)
                score += 1;
            else
                break;
        }
...
int QuoridorCore_other_compareWall(const void *first, const void *second)
{
    TurnToSort a = *(TurnToSort *)first;
    TurnToSort b = *(TurnToSort *)second;

    if (a.turn.action == QUORIDOR_ACTION_UNDEFINED)
        return 1;
    if (b.turn.action == QUORIDOR_ACTION_UNDEFINED)
        return -1;
...
static float QuoridorCore_other_minMax(
    ...
    float alpha, float beta, QuoridorTurn *turn, void *AIdata)
{
    ...
    if (currDepth >= maxDepth)
        return QuoridorCore_other_computeScore(self, playerID, *turn);
    ...
    QuoridorCore_other_getShortestPath(self, playerID, my_path.tiles, &(my_path.size));
    QuoridorCore_other_getShortestPath(self, playerID ^ 1, other_path.tiles, &(other_path.size));
    ...
    for (int i = 0; i < gridSize; i++)
    {
        for (int j = 0; j < gridSize; j++)
        {
            ...
            if (QuoridorCore_canPlayWall(&gameCopy, WALL_TYPE_VERTICAL, i, j))
            {
                ...
                list[pos].turn.i = i;
                list[pos].turn.j = j;
                list[pos].value = QuoridorCore_other_computeWall(gameCopy, playerID, my_path, other_path, list[pos].turn);
                pos++;
            }
    ...
    for (int i = 0; i < movesPos; i++)
    {
        ...
        currValue = QuoridorCore_other_minMax(&gameCopy, playerID, currDepth + 1, maxDepth, alpha, beta, &childTurn, AIdata);
        gameCopy = *self;
    ...
    if (pos > 0)
    {
        qsort(list, pos, sizeof(TurnToSort), QuoridorCore_other_compareWall);
...
#if DEBUG == 0
    float childValue = QuoridorCore_other_minMax(self, self->playerID, 0, 5, alpha, beta, &childTurn, aiData);
#endif
