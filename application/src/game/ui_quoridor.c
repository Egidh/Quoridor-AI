...
void UIQuoridor_updateTurn(UIQuoridor *self)
{
    ...
    if (playerTurn == false)
    {
        ...
        if (self->m_aiTurn.action == QUORIDOR_ACTION_UNDEFINED)
        {
            ...
            if (core->playerID == 1)
                ...
                self->m_aiTurn = QuoridorCore_computeOtherTurn(core, depth, self->m_aiData[core->playerID]);
        }
        else
        {
    ...
    else
    {
        ...
        if (input->validatePressed == false)
            return;
...
void UIQuoridor_updateRects(UIQuoridor *self)
{
    ...
    const float settingsH = (float)Text_getHeight(self->m_textTitleSettings) + listCount * (buttonH + listSep) + titleSep;
...
void UIQuoridor_renderPageReview(UIQuoridor *self)
{
    ...
    if (core->state != QUORIDOR_STATE_UNFINISHED)
        UIButton_render(self->m_buttonNextStep);
